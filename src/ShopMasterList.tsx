...
const createShopMaster = `
  mutation CreateShopMaster($input: CreateShopMasterInput!) {
    createShopMaster(input: $input) {
      id
      name
      sort
      disabled
...
const updateShopMaster = `
  mutation UpdateShopMaster($input: UpdateShopMasterInput!) {
    updateShopMaster(input: $input) {
      id
      name
      sort
      disabled
...
const ShopMasterList: React.FC = () => {
  ...
  const fetchMasters = async () => {
    ...
    const items = json.data?.listShopMasters?.items ?? [];
  ...
  const handleDisable = async (id: string) => {
    if (window.confirm('この店舗を「不要」扱いにしますか？（登録画面で選択できなくなります）')) {
      await fetch(APPSYNC_ENDPOINT, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <input value={name} onChange={e => setName(e.target.value)} placeholder="店舗名" required />

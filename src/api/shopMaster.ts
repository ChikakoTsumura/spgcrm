...
// 一覧取得
export async function listShopMasters() {
  const query = `
  ...
  return json.data?.listShopMasters?.items || [];
}

// 登録
export async function createShopMaster(name: string, sort: number) {
  const mutation = `
    mutation CreateShopMaster($input: CreateShopMasterInput!) {
      createShopMaster(input: $input) { id name sort }
    }
  `;

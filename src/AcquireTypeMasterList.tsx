...
import { listAcquireTypeMasters, createAcquireTypeMaster, updateAcquireTypeMaster } from './api/acquireTypeMaster';

const AcquireTypeMasterList: React.FC = () => {
  ...
  const fetchMasters = async () => {
    const items = await listAcquireTypeMasters();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await createAcquireTypeMaster(name, sort);
    setName('');
    setSort(0);
    fetchMasters();
  };
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (editId) {
      await updateAcquireTypeMaster(editId, { name: editName, sort: editSort });
      setEditId(null);
      setEditName('');
      setEditSort(0);
      fetchMasters();
  ...
  const handleDisable = async (id: string) => {
    if (window.confirm('この区分を「不要」扱いにしますか？（登録画面で選択できなくなります）')) {
      await updateAcquireTypeMaster(id, { disabled: true });
      fetchMasters();
    }
  };
...
export default AcquireTypeMasterList;
